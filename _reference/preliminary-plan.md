...
**Depends on:** Item 6.
**Interview hook:** "How do you avoid training-serving skew?" + "Spot the bug." (the killer talking-point) + "How do you handle unseen categories at serve time?" Answer: "The `FeatureTransformer` is designed to be fitted only on the training data, and then used to transform both train and test. This way, we avoid data leakage. If we fit it on the full dataset before splitting, we would be leaking information from the test set into the training process, which can lead to overly optimistic performance metrics. To handle unseen categories at serve time, we can use an encoder that has an 'unknown' category or simply ignore unseen categories, depending on the use case."
